...
#define TAG "EXAMPLE"
#define BUTTON_GPIO 27  // Pino do botão (ajuste conforme sua placa)
...
static void prov_complete(uint16_t net_idx, uint16_t addr, uint8_t flags, uint32_t iv_index)
{
    ...
    ESP_LOGI(TAG, "Provisioning completado! NetIdx: 0x%04x, Addr: 0x%04x", net_idx, addr);
    board_led_operation(LED_G, LED_OFF);
    //gpio_set_level(LED_GPIO_PIN, onoff);
...
void example_ble_mesh_send_gen_onoff_set(void)
{
    ...
    common.ctx.net_idx = store.net_idx;    // Usa o NetIdx provisionado
    common.ctx.app_idx = store.app_idx;    // Usa o AppIdx configurado
    common.ctx.addr = 0x003A;             // Endereço ESPECÍFICO do nó servidor (substitua pelo correto)
    common.ctx.send_ttl = 3;              // Time-to-Live padrão
    common.msg_timeout = 0;               // Usa timeout padrão

    // Configura o estado ON/OFF
    ...
    if (event == ESP_BLE_MESH_CFG_SERVER_STATE_CHANGE_EVT) {
        ...
        case ESP_BLE_MESH_MODEL_OP_MODEL_APP_BIND:
            ...
            if (param->value.state_change.mod_app_bind.model_id == ESP_BLE_MESH_MODEL_ID_GEN_ONOFF_CLI) {
                ...
                ESP_LOGI(TAG, "AppKey vinculada: AppIdx 0x%04x", store.app_idx);
...
// Handler da ISR (apenas marca o flag)
...
void init_button(void) {
    gpio_config_t btn_config = {
        ...
        .intr_type = GPIO_INTR_POSEDGE,   // Interrupção na borda de descida (botão pressionado)
    };
    gpio_config(&btn_config);
    gpio_install_isr_service(0);  // Instala o serviço de ISR
...
void app_main(void)
{
    ...
    while (1) {
        if (button_pressed) {
            button_pressed = false;  // Reseta o flag
            example_ble_mesh_send_gen_onoff_set();  // Envia comando BLE Mesh
            vTaskDelay(pdMS_TO_TICKS(20));  // Debounce simples (20ms)
        }
